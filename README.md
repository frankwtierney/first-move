# First Move — Facilitation Instincts Assessment

Single-page self-assessment web app for University at Buffalo Community Assistants,
taken live during training (~80 CAs per room, on phones, under 3 minutes).

See [`PRD.md`](./PRD.md) for full requirements, screens, scoring, branding, and
acceptance criteria.

## Status

Docs-only seed. App not yet built.

## Missing source of truth

The PRD references `Facilitation-Instincts-Assessment-Spec.md` (§4 items + §5 results
copy) as the **verbatim** content source. That spec file has not been added to this
repo yet and must be included before any UI work begins.
